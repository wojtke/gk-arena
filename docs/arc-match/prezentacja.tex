...
\begin{frame}{Wyniki: próg gry $k^*(n)$}
  ...
  \end{center}
  \medskip
  Próg $k^*(n)$ leży poniżej pułapu $\lceil n/2\rceil$ (czyli Breaker realnie przeszkadza); przy $n=3$
  wzorzec $k=2$ jest możliwy, lecz Maker nie potrafi go wymusić. Ogólne zachowanie $k^*(n)$ dla
  dużych $n$ pozostaje \alert{problemem otwartym}.
\end{frame}
