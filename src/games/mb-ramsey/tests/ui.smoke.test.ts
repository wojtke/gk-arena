...
// (New game, switch to hotseat, claim an edge, Hint, Undo) and assert nothing throws and the DOM
...
describe('MB Ramsey UI smoke (jsdom)', () => {
  it('mounts into slots, renders the board, and survives interactions without throwing', async () => {
    ...
    // Claim a board edge — a real move should update the DOM.
    ...
    const ownerCount = document.querySelectorAll('#board .edge.r').length;
    expect(ownerCount).toBeGreaterThanOrEqual(1); // a red edge now exists
